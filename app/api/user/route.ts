...
import { checkSingleAdminRule, requireAdmin } from "@/lib/auth"
...
export const PATCH = requireAdmin(async (request: Request): Promise<Response> => {
  try {
    ...
    // Vérifier si l'utilisateur existe - CORRECTION : _id au lieu de id
    const existingUser = await usersCollection.findOne({
      _id: new ObjectId(validatedData.id), // CORRECTION : _id
      deletedAt: { $exists: false }
    })
    ...
    // Mettre à jour dans la base - CORRECTION : _id au lieu de id
...
export const DELETE = requireAdmin(async (request: Request): Promise<Response> => {
  try {
    ...
    // Vérifier si l'utilisateur existe - CORRECTION : _id
    const existingUser = await usersCollection.findOne({
      _id: new ObjectId(validatedData.id), // CORRECTION : _id
      deletedAt: { $exists: false }
    })
