...
// Vérifier les variables d'environnement (uniquement en production et pas pendant le build)
const isProduction =
...
if (isProduction) {
  ...
  requiredEnvVars.forEach(varName => {
    if (!process.env[varName]) {
      ...
      throw new Error(
        `La variable ${varName} est requise pour le service email en production`
      );
    }
  });
}
...
// Configuration du transporteur SMTP
  export const transporter = process.env.EMAIL_HOST
  ...
  export const verifyEmailConnection = async (): Promise<boolean> => {
    if (!transporter) {
      console.warn(' Service email non configuré - Variables EMAIL_* manquantes');
      return false;
    }
...
export const sendEmail = async (options: EmailOptions): Promise<boolean> => {
  if (!transporter) {
    console.warn(
      ' Service email non configuré - Email non envoyé à',
      options.to
    );
    return false;
  }
  ...
  try {
    const mailOptions = {
      from: `"Ambassade" <${process.env.EMAIL_USER}>`,
      ...options,
    };
    ...
    <div class="content">
      ...
      <div class="next-steps">
        ...
        <ul>
          ...
          <li> Délai de réponse : 24-48 heures</li>
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="important-note">
        <strong> Important :</strong> Ne répondez pas à cet email si vous n'êtes pas l'expéditeur original du message.
      </div>
...
export const confirmMessageToSender = async (
